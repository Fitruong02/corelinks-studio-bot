...
export const messageUpdateEvent = {
  ...
  once: false,
  async execute(oldMessage: Message | PartialMessage, newMessage: Message | PartialMessage, bot: CorelinksBot) {
    try {
...
export const messageDeleteEvent = {
  name: DiscordEvents.MessageDelete,
  once: false,
  async execute(message: Message | PartialMessage, bot: CorelinksBot) {
    try {
      // Ignore bot messages
