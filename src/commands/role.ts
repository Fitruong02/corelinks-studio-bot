...
async function handleRoleInfo(interaction: ChatInputCommandInteraction, roleManager: RoleManager): Promise<void> {
  try {
    ...
    infoEmbed.addFields(
      ...
      { name: 'Color', value: 'hexColor' in role ? role.hexColor : 'N/A', inline: true },
      { name: 'Position', value: 'position' in role ? role.position.toString() : 'N/A', inline: true },
      { name: 'Mentionable', value: 'mentionable' in role ? (role.mentionable ? 'Yes' : 'No') : 'N/A', inline: true },
      { name: 'Hoisted', value: 'hoist' in role ? (role.hoist ? 'Yes' : 'No') : 'N/A', inline: true },
      { name: 'Members', value: 'members' in role ? role.members.size.toString() : 'N/A', inline: true },
      { name: 'Created', value: 'createdTimestamp' in role ? `<t:${Math.floor((role as any).createdTimestamp / 1000)}:R>` : 'N/A', inline: true }
    );
