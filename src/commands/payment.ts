...
import { PaymentManager } from '@modules/payment/index';
    ...
    async execute(interaction: ChatInputCommandInteraction, bot: CorelinksBot) {
      if (!FeatureManager.isEnabled('paymentSystem')) {
        const disabledEmbed = EmbedManager.createErrorEmbed(
          'Feature Disabled',
          'The payment system is currently disabled.'
        );
...
async function handleInvoiceCreate(interaction: ChatInputCommandInteraction, paymentManager: PaymentManager): Promise<void> {
  try {
    if (!PermissionManager.isStaff(interaction.member as any)) {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Permission Denied',
        'Only staff members can create invoices.'
      );
    ...
    // Validate input
    if (!ValidationManager.isValidCustomerId(customerId)) {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Invalid Customer ID',
        'Please provide a valid customer ID (format: CUST-XXXX).'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
      return;
    }
    ...
    if (isDeposit && (!depositAmount || depositAmount >= amount)) {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Invalid Deposit',
        'Deposit amount must be less than the total amount.'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
      return;
    }
    ...
    const invoice = await paymentManager.createInvoice(
      interaction.user.id,
      customerId,
      productName,
      amount,
    ...
    if (invoice) {
      const successEmbed = EmbedManager.createSuccessEmbed(
        'Invoice Created',
        `Invoice ${invoice.invoiceId} has been created and sent to the customer.`
      );

      successEmbed.addFields(
        { name: 'Product', value: productName, inline: true },
        { name: 'Amount', value: ValidationManager.formatCurrency(amount), inline: true },
...
async function handleInvoiceView(interaction: ChatInputCommandInteraction, paymentManager: PaymentManager): Promise<void> {
  try {
    ...
    // Check permissions
    const isStaff = PermissionManager.isStaff(interaction.member as any);
    ...
    invoiceEmbed.addFields(
      ...
      { name: 'Payment Type', value: invoice.isDeposit ? 'Deposit' : 'Full Payment', inline: true },
      { name: 'Created', value: `<t:${Math.floor(invoice.createdAt.getTime() / 1000)}:F>`, inline: true }
    );
...
async function handleRefundRequest(interaction: ChatInputCommandInteraction, paymentManager: PaymentManager): Promise<void> {
  try {
    const invoiceId = interaction.options.getString('invoice-id', true);
    ...
    if (success) {
      const successEmbed = EmbedManager.createSuccessEmbed(
        'Refund Requested',
        `Your refund request for invoice ${invoiceId} has been submitted for review.`
      );
      await interaction.reply({ embeds: [successEmbed], ephemeral: true });
    } else {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Request Failed',
        'Failed to submit refund request. Please check the invoice ID or try again later.'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
    }
...
async function handleRefundApprove(interaction: ChatInputCommandInteraction, paymentManager: PaymentManager): Promise<void> {
  try {
    if (!PermissionManager.isStaff(interaction.member as any)) {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Permission Denied',
        'Only staff members can approve refunds.'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
      return;
    }
    ...
    if (success) {
      const successEmbed = EmbedManager.createSuccessEmbed(
        'Refund Approved',
        `Refund for invoice ${invoiceId} has been approved and processed.`
      );
      await interaction.reply({ embeds: [successEmbed], ephemeral: true });
    } else {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Approval Failed',
        'Failed to approve the refund. Please check the invoice ID.'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
    }
    ...
    const errorEmbed = EmbedManager.createErrorEmbed(
      'Error',
      'An error occurred while approving the refund.'
    );
    await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
  }
}

async function handleRefundDeny(interaction: ChatInputCommandInteraction, paymentManager: PaymentManager): Promise<void> {
  try {
    if (!PermissionManager.isStaff(interaction.member as any)) {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Permission Denied',
        'Only staff members can deny refunds.'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
      return;
    }
    ...
    if (success) {
      const successEmbed = EmbedManager.createSuccessEmbed(
        'Refund Denied',
        `Refund for invoice ${invoiceId} has been denied.`
      );
      await interaction.reply({ embeds: [successEmbed], ephemeral: true });
    } else {
      const errorEmbed = EmbedManager.createErrorEmbed(
        'Denial Failed',
        'Failed to deny the refund. Please check the invoice ID.'
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
    }
    ...
    const errorEmbed = EmbedManager.createErrorEmbed(
      'Error',
      'An error occurred while denying the refund.'
    );
    await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
  }
}
